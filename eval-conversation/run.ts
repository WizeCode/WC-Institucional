...
Respostas curtas, linguagem informal. Orçamento: não sabe. Prazo: sem urgência.`,
    },
    {
        ...
        firstMessage:
            "quanto custa fazer um aplicativo de delivery? preciso urgente",
...
async function runConversation(persona: Persona): Promise<void> {
    ...
    while (turnNum < MAX_TURNS) {
        ...
        const wizeResponse = await callModel(
            WIZE_MODEL,
            systemPrompt,
            wizeHistory,
            sessionId
    ...
    writeFileSync(
        ...
        buildReport(persona, turns, briefingRaw, evaluation)
    )
    console.log(
...
async function main() {
    ...
    if (toRun.length === 0) {
        ...
        for (const p of personas)
            console.error(`  ${c.cyan}${p.id}${c.reset}  — ${p.label}`)
        console.error()
        process.exit(1)
    }
